...
describe("ReMonsterShop", function () {
    ...
    async function deployShopFixture() {
        ...
        const shop = await Shop.connect(owner).deploy(owner.address, generalHash.address, genesisHash.address, generalHash.address, 1,1,1,1);
        shop.deployed();
        ...
        return { shop, addressReceice, owner, managerAccount, otherAccount1 , generalHash, genesisHash};
    }
    ...
    //     });
    // });
    describe("ssssssssssssssssssssssssssss", function () {
        it("Should emit an event on buyAsset ", async function () {
            const { shop, owner, otherAccount1, generalHash, genesisHash } = await loadFixture(
                deployShopFixture
            );
            let xxxRole = await generalHash.connect(owner).MANAGEMENT_ROLE();
            await generalHash.grantRole(xxxRole, shop.address);

            let xxxsRole = await genesisHash.connect(owner).MANAGEMENT_ROLE();
            await genesisHash.grantRole(xxxsRole, shop.address);
            await expect(shop.buyItem(1,1,1,1)).not.to.be.reverted;
