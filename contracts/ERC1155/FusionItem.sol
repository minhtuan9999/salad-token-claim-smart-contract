...
// File contracts/ERC1155/FusionItem.sol
...
contract FusionItem is ERC1155, AccessControl, Ownable {
    ...
    function getDetailItems(uint256[] memory listIds) public view returns(ITEM_DETAIL[] memory) {
        ITEM_DETAIL[] memory data = new ITEM_DETAIL[](listIds.length);
        for (uint i = 0; i < listIds.length; i++) {
            data[i] = itemDetail[listIds[i]];
        }
        return data;
    }
