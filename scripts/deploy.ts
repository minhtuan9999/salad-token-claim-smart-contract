...
async function main() {
  ...
  let tokenXXXs = process.env.ADDRESS_CONTRACT_TOKEN_XXX as string;
  let generals = process.env.ADDRESS_CONTRACT_GENERAL as string;
  let genesiss = process.env.ADDRESS_CONTRACT_GENESIS as string;
  let hashChipNFTs = process.env.ADDRESS_CONTRACT_HASHCHIP as string;
  let monsterMemors = process.env.ADDRESS_CONTRACT_MEMORY as string;
  let regenerationItems = process.env.ADDRESS_CONTRACT_REGENERATION_ITEM as string;
  let fusionItems = process.env.ADDRESS_CONTRACT_FUSION_ITEM as string;
  let monsters = process.env.ADDRESS_CONTRACT_MONSTER as string;
  ...
  const regenFusion = await RegenFusion.deploy();
  regenFusion.deployed(); 
  regenFusion.initContractAddress(
    tokenXXXs, 
    generals, 
    genesiss, 
    hashChipNFTs, 
