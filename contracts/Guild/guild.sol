...
// File contracts/Guild/guild.sol
...
{   
    ...
    mapping(uint8 => uint256) public costType;
    mapping(bytes => bool) public _isSigned;

    uint8 CREATE_GUILD = 0;
    uint8 RENAME_GUILD = 1;
    
    event setGuilds(address from, uint8 guild );
    constructor(IToken tokenBase){
        _setRoleAdmin(MANAGEMENT_ROLE, MANAGEMENT_ROLE);
        _setupRole(MANAGEMENT_ROLE, _msgSender());
        validator = msg.sender;
        tokenBaseContract = tokenBase;
        costType[CREATE_GUILD] = 10;
        costType[RENAME_GUILD] = 5;
    }
