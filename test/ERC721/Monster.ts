...
describe("Monster Managerment", function () {
    ...
    //     //     await expect(monster.connect(owner).fusionMonsterNFT(owner.address, token2, token3, [0], [0])).not.to.be.reverted;
    ...
    //     //     await monster.connect(owner).initSetTreasuryAdress(treasuryAddress.address);
    //     //     await monster.connect(owner).initSetTokenBaseContract(tokenXXX.address);
    //     //     await monster.connect(owner).initSetMonsterItemContract(remonsterItem.address);
    //     //     let xxxRole = await tokenXXX.connect(owner).MANAGEMENT_ROLE();
    //     //     await tokenXXX.connect(owner).grantRole(xxxRole, monster.address);
    ...
    //     //     let tx2 = await monster.connect(owner).mintMonsterFromRegeneration(itemToken);
    //     //     const receipt2 = await tx2.wait();
    //     //     let token2;
    //     //     if (receipt2 && receipt2.events) {
    //     //         const events = receipt2.events;
    ...
    //     //     let tx3 = await monster.connect(owner).mintMonsterFromRegeneration(itemToken);
    //     //     const receipt3 = await tx3.wait();
    //     //     let token3;
    //     //     if (receipt3 && receipt3.events) {
    //     //         const events = receipt3.events;
    ...
    //     //         }
    //     //     }
    //     //     let balanceItemBefore = await remonsterItem.balanceOf(owner.address, itemToken)
    //     //     await expect(monster.connect(owner).fusionMonsterNFT(owner.address, token2, token3, Array.of(itemToken), Array.of(11))).to.be.revertedWith("ERC1155: burn amount exceeds balance");
    //     //     await expect(monster.connect(owner).fusionMonsterNFT(owner.address, token2, token3, Array.of(itemToken), Array.of(numberFusionItem))).not.to.be.reverted;
    //     //     let balanceItemAfter = await remonsterItem.balanceOf(owner.address, itemToken)
            
    //     //     expect(balanceItemBefore).to.equal(parseInt(balanceItemAfter.toString()) + numberFusionItem);
    //     //     expect(await monster.connect(owner).balanceOf(owner.address)).to.equals(1);
    //     //     let listTokenAfter = await monster.getListTokenOfAddress(owner.address);

    //     //     expect(listTokenAfter.toString()).to.eql((parseInt(token3)+1).toString());

    //     //     // multiple item fusion
    //     //     let tx4 = await remonsterItem.connect(owner).mint(owner.address,2,2,number,"0x32");
    ...
    //     //     let tx5 = await monster.connect(owner).mintMonsterFromRegeneration(itemToken);
    ...
    //     //     let tx6 = await monster.connect(owner).mintMonsterFromRegeneration(itemToken);
    ...
    //     //         }
    //     //     }
    //     //     let balanceItemBefore1 = await remonsterItem.balanceOf(owner.address, itemToken2)
    //     //     await expect(monster.connect(owner).fusionMonsterNFT(owner.address, monsterId1, monsterId2, Array.of(itemToken, itemToken2), Array.of(numberFusionItem,numberFusionItem))).not.to.be.reverted;
    //     //     let balanceItemAfter1 = await remonsterItem.balanceOf(owner.address, itemToken2)

    //     //     expect(balanceItemBefore1).to.equal(parseInt(balanceItemAfter1.toString()) + numberFusionItem);
    //     // });
    ...
    //         let tx4 = await monster.connect(owner).fusionMonsterNFT(owner.address, token2, token3, [0], [0]);
    //         const receipt4 = await tx4.wait();
    //         let token4;
    //         if (receipt4 && receipt4.events) {
    //             const events = receipt4.events;
    //             const createEvent = events.find((event) => event.event === "fusionMultipleMonster");
    //             if (createEvent && createEvent.args) {
    ...
    describe("Fusion Monster ", async function () {
        it('Should check function: fusionMonster: Dont using fusion item', async function () {
            const { monster, monsterMemory, owner, userAddress, tokenXXX, remonsterItem, treasuryAddress, genesisHash } = await loadFixture(deployERC721Fixture);
            await monster.connect(owner).initSetRegenerationContract(remonsterItem.address);
            await monster.connect(owner).initSetValidator(owner.address);
            await monster.connect(owner).initSetTreasuryAdress(treasuryAddress.address);
            await monster.connect(owner).initSetTokenBaseContract(tokenXXX.address);
            ...
            let tx1 = await genesisHash.connect(owner).createNFT(owner.address,group);
